...
function toCleanup(dispose) {
  if (typeof dispose === "function") {
    return dispose;
  }

  if (typeof dispose?.disconnect === "function") {
...
async function runOptionalVisualStep(label, importer, callback) {
  try {
    const module = await importer();
    return toCleanup(callback(module));
  } catch (error) {
    console.error("[showcase-visuals] " + label + " failed", error);
    return noop;
  }
}

function normalizeDispose(dispose) {
  return toCleanup(dispose);
}
...
export async function initShowcaseVisuals(root = document) {
  const scope = typeof root !== "undefined" ? root : document;
  const optionalDisposers = await Promise.all([
    runOptionalVisualStep(
      "initShowcaseMediaScenes",
      () => import("../../visuals/dom/showcase-media-scenes.js"),
      (module) => module.initShowcaseMediaScenes(scope),
    ),
    runOptionalVisualStep(
      "initCvScrollRows",
      () => import("../../visuals/dom/showcase-scroll-row.js"),
      (module) => module.initCvScrollRows(scope),
    ),
  ]);

  const visualTargets = [...scope.querySelectorAll("[data-visual-demo]")];
  const animationPreviews = [...scope.querySelectorAll("[data-animation]")];
  const sliders = [...scope.querySelectorAll("[data-showcase-preview-slider]")];

  const sliderDisposers = sliders.map(initPreviewSlider);

  const stopAnimationObserver = toCleanup(
    observeOnceVisible(
      animationPreviews,
      (preview) => {
  ...
  const stopVisualObserver = toCleanup(
    observeOnceVisible(
      visualTargets,
      (target) => {
  ...
  return () => {
    ...
    sliderDisposers.forEach((dispose) => toCleanup(dispose)());
    optionalDisposers.forEach((dispose) => toCleanup(dispose)());
  };
}
