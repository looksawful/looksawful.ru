...
export const mountRowMarquee = async (canvasId, options = {}) => {
  ...
  const render = () => {
    ...
    if (images.length) {
      const ratio = parseRatio(config.ratio);
      const cardHeight = Math.min(height * 0.72, 260);
      const cardWidth = cardHeight * ratio;
      const gap = cardHeight * 0.14;
      const cycle = images.length * (cardWidth + gap);
      const offset = ((frame * 0.42 * config.speed) % cycle);
      const y = (height - cardHeight) / 2;

      for (let i = -images.length; i < images.length * 2; i++) {
        const image = images[((i % images.length) + images.length) % images.length];
        const x = width - offset + i * (cardWidth + gap);
        if (x > width + cardWidth || x < -cardWidth) continue;
        ctx.save();
        roundedClip(ctx, x, y, cardWidth, cardHeight, 18);
        ctx.clip();
        fitCover(ctx, image, x, y, cardWidth, cardHeight);
        ctx.restore();
        ctx.strokeStyle = "#fff";
        ctx.lineWidth = 1;
        roundedClip(ctx, x, y, cardWidth, cardHeight, 18);
