...
const VENUS_FRAME_WIDTH = 192;
const VENUS_FRAME_HEIGHT = 208;
const VENUS_ANCHOR = { x: 0.5, y: 1 } as const;

interface VenusClipOptions {
  row: number;
  frameCount: number;
...
function createClip(src: string, options: VenusClipOptions): SpriteAnimationDefinition {
  return {
    src,
    frameWidth: VENUS_FRAME_WIDTH,
    frameHeight: VENUS_FRAME_HEIGHT,
    frameCount: options.frameCount,
    fps: 1000 / options.frameDurationMs,
    loop: options.loop,
    sourceX: 0,
    sourceY: options.row * VENUS_FRAME_HEIGHT,
    anchor: VENUS_ANCHOR,
  };
}

export function createVenusSpriteManifest(src: string): SpriteManifest {
