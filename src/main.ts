...
function initBeforeAfter(root: HTMLElement): Destroy {
  const range = root.querySelector(".before-after__range");
  if (!(range instanceof HTMLInputElement)) return noop;
  const render = (): void => root.style.setProperty("--before-after-split", `${range.value}%`);
  range.addEventListener("input", render, { passive: true });
  render();
  return () => range.removeEventListener("input", render);
}

function initViewportAutoplayVideos(root: ParentNode | null | undefined = document): Destroy {
...
const destroys: Destroy[] = [destroySiteAnalyticsGoalTracking, destroySiteAnalyticsConsent];
let destroyed = false;
