...
export const DIAGONAL_DEFAULTS = Object.freeze({
  rows: 6,
  gap: 8,
  speed: 0.035,
  rowSpeeds: null,
  ...
  for (let cycleStart = start; cycleStart < plane.width + cycle; cycleStart += cycle) {
    ...
    for (let tileIndex = 0; tileIndex < row.tiles.length; tileIndex += 1) {
      ...
      if (x < plane.width && x + tile.width > 0) {
        const baseRegion = { key: `diagonal:${rowIndex}:${tileIndex}:${copyIndex}`, item: tile.item, width: tile.width, height: rowHeight };
...
export async function mountDiagonal(target, userOptions = {}) {
  const canvas = resolveCanvas(target);
  if (!canvas) throw new Error("mountDiagonal: canvas not found.");
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("mountDiagonal: 2D context is unavailable.");
  activeInstances.get(canvas)?.dispose();

  let options = mergeOptions(DIAGONAL_DEFAULTS, userOptions);
  let items = await loadItems(options.items, "Diagonal");
  let layout = null;
  let requestRender = () => {};
  ...
  const render = ({ elapsed, width, height, reducedMotion }) => {
    ...
    current.rows.forEach((row, rowIndex) => {
      ...
      const initialOffset = options.rowOffsets?.[rowIndex] ?? rowIndex * 173;
  ...
  const controller = {
    ...
    async update(nextOptions = {}) {
      ...
      if (Object.hasOwn(nextOptions, "items")) items = await loadItems(nextOptions.items, "Diagonal");
      layout = null;
      loop.redraw();
      return controller;
    },
