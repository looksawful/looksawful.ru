...
export const SHOWCASE_DIAGONAL_DEFAULTS = Object.freeze({
  rows: 5,
  gap: 8,
  speed: 0.026,
  rowSpeeds: null,
  ...
  for (let cycleStart = start; cycleStart < plane.width + cycle; cycleStart += cycle) {
    ...
    for (let tileIndex = 0; tileIndex < row.tiles.length; tileIndex += 1) {
      ...
      if (x < plane.width && x + tile.width > 0) {
        const baseRegion = { key: `showcase-diagonal:${rowIndex}:${tileIndex}:${copyIndex}`, item: tile.item, width: tile.width, height: rowHeight };
...
export async function mountShowcaseDiagonal(target, userOptions = {}) {
  const canvas = resolveCanvas(target);
  if (!canvas) throw new Error("mountShowcaseDiagonal: canvas not found.");
  const ctx = canvas.getContext("2d");
  if (!ctx) throw new Error("mountShowcaseDiagonal: 2D context is unavailable.");
  activeInstances.get(canvas)?.dispose();

  let options = mergeOptions(SHOWCASE_DIAGONAL_DEFAULTS, userOptions);
  let items = await loadItems(options.items, "Showcase diagonal");
  let layout = null;
  let requestRender = () => {};
  ...
  const render = ({ elapsed, width, height, reducedMotion }) => {
    ...
    current.rows.forEach((row, rowIndex) => {
      ...
      const initialOffset = options.rowOffsets?.[rowIndex] ?? rowIndex * 191;
  ...
  const controller = {
    ...
    async update(nextOptions = {}) {
      ...
      if (Object.hasOwn(nextOptions, "items")) items = await loadItems(nextOptions.items, "Showcase diagonal");
      layout = null;
      loop.redraw();
      return controller;
    },
