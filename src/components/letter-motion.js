...
function getGsap() {
  return typeof window !== "undefined" ? window.gsap || null : null;
}
...
function canAnimate() {
  return Boolean(getGsap()) && !shouldReduceMotion();
}

function normalizeProfile(profile) {
  if (typeof profile === "string") {
    return PROFILE_CONFIG[profile] || PROFILE_CONFIG.heading;
  }

  if (!profile || typeof profile !== "object") {
    return PROFILE_CONFIG.heading;
  }

  return {
    ...PROFILE_CONFIG.heading,
    ...profile,
  };
}

function getLetters(target, options = {}) {
  if (Array.isArray(options.letters)) {
    return options.letters.filter(Boolean);
  }

  if (options.letters && typeof options.letters.length === "number") {
    return [...options.letters].filter(Boolean);
  }
...
function getLetterByEvent(letters, event) {
  ...
  if (Number.isFinite(event.index)) {
    return letters[Math.abs(Math.trunc(event.index)) % letters.length];
  }
...
function createEventTimeline(gsap, letter, event) {
  ...
  if (event.type === "flip") {
    timeline.add(createFlip(gsap, letter, event), 0);
  } else if (event.type === "wobble") {
    timeline.add(createWobble(gsap, letter, event), 0);
  } else if (event.type === "stretch") {
    timeline.add(createStretch(gsap, letter, event), 0);
  } else {
    timeline.add(createJump(gsap, letter, event), 0);
  }
...
export function createLetterIdleMotion(target, options = {}) {
  if (!target || motionStore.has(target) || !canAnimate()) {
    return null;
  }
  ...
  if (!letters.length || !events.length) {
    return null;
  }

  gsap.set(letters, {
    transformOrigin: profile.transformOrigin || "50% 58%",
    transformPerspective: profile.transformPerspective || 900,
    force3D: profile.force3D !== false,
  });

  const timelines = events
    .map((event) => createEventTimeline(gsap, getLetterByEvent(letters, event), event))
    .filter(Boolean);
