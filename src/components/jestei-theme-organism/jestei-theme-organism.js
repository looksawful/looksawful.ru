 ...
 */
function readModelUrl(root) {
  return root?.dataset?.jesteiThemeModelSrc || JESTEI_THEME_MODEL_URL;
}

function readDracoPath(root) {
  return root?.dataset?.jesteiThemeDracoPath || JESTEI_THEME_DRACO_PATH;
}

function getModelBuffer(modelUrl = JESTEI_THEME_MODEL_URL) {
  const normalizedUrl = modelUrl || JESTEI_THEME_MODEL_URL;
  let modelBufferPromise = modelBufferPromises.get(normalizedUrl);

  if (!modelBufferPromise) {
    modelBufferPromise = fetch(normalizedUrl).then((response) => {
      if (!response.ok) {
    ...
    modelBufferPromises.set(normalizedUrl, modelBufferPromise);

    modelBufferPromise.catch(() => {
      modelBufferPromises.delete(normalizedUrl);
    });
  }
...
export function preloadJesteiThemeOrganismAssets({ modelUrl } = {}) {
  return Promise.all([getAnimatedRuntime(), getModelBuffer(modelUrl)]).then(noop);
}
...
function createTrackController(elements) {
  ...
  function resize() {
    const measuredWidth = themeCopyShell.offsetWidth;
...
async function loadNormalizedModel({ THREE, GLTFLoader, DRACOLoader }, { modelUrl, dracoPath } = {}) {
  const dracoLoader = new DRACOLoader();
  dracoLoader.setDecoderPath(dracoPath || JESTEI_THEME_DRACO_PATH);

  const loader = new GLTFLoader();
  loader.setDRACOLoader(dracoLoader);

  try {
    const sourceBuffer = await getModelBuffer(modelUrl);
...
async function createAnimatedExperience(root, elements, track, { onFatalError } = {}) {
  ...
  const { model, localBounds } = await loadNormalizedModel({
    ...
    modelUrl: readModelUrl(root),
    dracoPath: readDracoPath(root),
  });
...
export function createJesteiThemeOrganism({ root, motion, sceneRuntime = null } = {}) {
  ...
  const hasSceneRuntime = Boolean(sceneRuntime);
  let sceneActive = hasSceneRuntime ? false : true;
  ...
  function shouldPrepare() {
    return !destroyed && motionAllowed && root.isConnected && (prepareRequested || visible || (hasSceneRuntime && sceneActive));
  }
  ...
  const api = Object.freeze({
    ...
    preload() {
      ...
      return preloadJesteiThemeOrganismAssets({ modelUrl: readModelUrl(root) }).catch((error) => {
