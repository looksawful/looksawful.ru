...
function init() {
  ...
  const particles = Array.from({ length: TOTAL }, (_, i) => {
    ...
    // tier for non-Druk: always small or medium only (no large)
    // tier for Druk: mostly small/medium, rare large (only every ~8th Druk particle)
    let tier;
    if (isDruk) {
  ...
  function draw() {
    particles.sort((a, b) => a.scale - b.scale); // z-sort: smaller scale draws first
