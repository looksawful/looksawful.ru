...
export function mountPortfolioPet(
  ...
  launcher.setAttribute("aria-label", "Открыть форму связи");
  ...
  const syncPetControls = (): void => {
    ...
    const left = Math.max(
      viewportRect.x + MIN_SAFE_MARGIN,
      Math.min(rect.right - 26, viewportRect.x + viewportRect.width - 28 - MIN_SAFE_MARGIN),
    );
    const top = Math.max(
      viewportRect.y + MIN_SAFE_MARGIN,
      Math.min(rect.top + 8, viewportRect.y + viewportRect.height - 28 - MIN_SAFE_MARGIN),
    );
    dismissButton.style.left = `${Math.round(left)}px`;
    dismissButton.style.top = `${Math.round(top)}px`;
  };
