...
type Destroy = () => void;
type InfiniteReelOptions = { motion?: MotionPreference };
type InfiniteReelsOptions = InfiniteReelOptions & { root?: ParentNode };
...
const animationMinInlineSize = (): number => {
...
function readPositiveNumber(element: HTMLElement, property: string): number | null {
  const value = Number.parseFloat(getComputedStyle(element).getPropertyValue(property));
  return Number.isFinite(value) && value > 0 ? value : null;
}

function getCycleDistance(track: HTMLElement): number | null {
...
export function createInfiniteReel(root: Element | null | undefined, { motion }: InfiniteReelOptions = {}): Destroy {
  ...
  const isActive = () => mounted && allowed && wideEnough && nearViewport && !document.hidden;
  const autoplayVideos = (): HTMLVideoElement[] => [...track.querySelectorAll("video[autoplay]")].filter(
    (video): video is HTMLVideoElement => video instanceof HTMLVideoElement && !video.closest("[data-media-deck]"),
  );
  ...
  const restoreDuration = () => {
    if (authoredDuration) { root.style.setProperty(DURATION_PROPERTY, authoredDuration, authoredDurationPriority); return; }
    root.style.removeProperty(DURATION_PROPERTY);
  };
  const syncDuration = () => {
    if (!mounted) return;
    const speed = readPositiveNumber(root, SPEED_PROPERTY);
    if (speed === null) { restoreDuration(); return; }
    const cycleDistance = getCycleDistance(track);
    if (cycleDistance === null) { restoreDuration(); return; }
    root.style.setProperty(DURATION_PROPERTY, `${(cycleDistance / speed).toFixed(3)}s`);
  };
  const syncPlayback = () => {
  ...
  const mount = () => {
    ...
    const items = [...track.children].filter((item): item is HTMLElement => item instanceof HTMLElement && !item.hasAttribute("data-infinite-reel-clone"));
  ...
  const reconcile = () => {
    if (destroyed) return;
    if (!allowed || !wideEnough) { if (mounted) unmount(); else syncPlayback(); return; }
    if (nearViewport) mount();
    syncPlayback();
  };
  const intersectionObserver = typeof IntersectionObserver === "function" ? new IntersectionObserver(([entry]) => {
    nearViewport = Boolean(entry?.isIntersecting); reconcile();
  }, { rootMargin: VIEWPORT_MARGIN, threshold: 0 }) : null;
  ...
  const resizeObserver = typeof ResizeObserver === "function" ? new ResizeObserver((entries) => {
    ...
    if (rootEntry) {
      ...
      if (nextWideEnough !== wideEnough) { wideEnough = nextWideEnough; reconcile(); }
    }
    if (mounted) syncDuration();
  }) : null;
  resizeObserver?.observe(root); resizeObserver?.observe(track);
  const handleVisibilityChange = () => syncPlayback();
  document.addEventListener("visibilitychange", handleVisibilityChange);
  const unsubscribe = motion?.subscribe(({ allowed: nextAllowed }) => { allowed = Boolean(nextAllowed); reconcile(); }) ?? noop;
  syncPlayback();
  if (!motion) reconcile();
  return () => {
    destroyed = true;
...
export function createInfiniteReels({ root = document, motion }: InfiniteReelsOptions = {}): Destroy {
  const destroys = [...root.querySelectorAll("[data-infinite-reel]")].map((element) => createInfiniteReel(element, { motion }));
