...
const mountViewer = async (root, entry) => {
  ...
  const renderModeHandlers = new Map();
  renderModeInputs.forEach((input) => {
    const handler = () => updateRenderMode(input.value);
    renderModeHandlers.set(input, handler);
  ...
  return () => {
    ...
    renderModeHandlers.forEach((handler, input) => input.removeEventListener("change", handler));
...
export default {
  title: "02 Molecules/Model Viewer/Logo Library",
  parameters: {
    layout: "padded",
    docs: {
      description: {
