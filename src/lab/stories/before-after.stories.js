...
export const ManualAdjusted = { parameters: { looksawful: { ...evidence("manual-adjusted").looksawful, state: "manual-adjusted" } }, play: async ({ canvasElement }) => {
  const root = canvasElement.querySelector("[data-before-after]");
  initBeforeAfter(root, { motion: motionPreference, autoReveal: false });
  const range = root.querySelector('input[type="range"]');
  expect(range).toBeInTheDocument();
...
export const AutoReveal = { parameters: { looksawful: { ...evidence("auto-reveal").looksawful, state: "auto-reveal" } }, play: async ({ canvasElement }) => {
  const root = canvasElement.querySelector("[data-before-after]");
