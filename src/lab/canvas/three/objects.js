...
const MODEL_URLS = {
  logo: new URL("../../assets/projects/jestei/logo/logo.glb", import.meta.url).href,
  laptop: new URL("../../assets/projects/jestei/macbook/model/laptop.glb", import.meta.url).href,
};

const RECORD_POOL_SCREEN_URL = new URL(
  "../../assets/projects/jestei/ui/record-pool-screen.png",
  import.meta.url,
).href;

const logoColors = [new THREE.Color("#000000")];
...
function addLaptopObjects(scene, renderer) {
  ...
  const screenTexture = new THREE.TextureLoader().load(RECORD_POOL_SCREEN_URL, (tex) => {
    tex.colorSpace = THREE.SRGBColorSpace;

    if (laptopGroup) {
      laptopGroup.traverse((child) => {
        if (child.isMesh && isScreenMesh(child)) {
          applyScreenTexture(child, tex);
        }
      });
    }
  });
