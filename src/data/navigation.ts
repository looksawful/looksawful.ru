...
export type NavigationLabelId = NavigationLabelPageId;
...
const navigationLabelIds = new Set<string>(NAVIGATION_LABEL_PAGE_IDS);
...
function parseNavigationLabel(value: unknown, index: number): NavigationLabelData {
  ...
  const id = NAVIGATION_LABEL_PAGE_IDS.find((candidate) => candidate === idValue);
...
export function parseNavigationLabels(value: unknown): readonly NavigationLabelData[] {
  ...
  for (const expectedId of NAVIGATION_LABEL_PAGE_IDS) {
    if (!byId.has(expectedId)) {
      throw new Error(`missing required navigation label id: ${expectedId}`);
    }
  }

  if (parsed.length !== NAVIGATION_LABEL_PAGE_IDS.length) {
    throw new Error(
      `navigation label count must remain ${NAVIGATION_LABEL_PAGE_IDS.length}; got ${parsed.length}`,
    );
  }

  return Object.freeze(NAVIGATION_LABEL_PAGE_IDS.map((id) => {
