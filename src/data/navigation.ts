...
export interface NavigationLabelData { id: NavigationLabelId; label: string; }
...
function parseNavigationLabel(value: unknown, index: number): NavigationLabelData {
  ...
  if (typeof idValue !== "string" || !navigationLabelIds.has(idValue)) throw new Error(`unexpected navigation label id: ${String(idValue)}`);
  ...
  if (typeof value.label !== "string" || value.label.trim().length === 0) throw new Error(`${itemLabel}.label must be a non-empty string`);
...
export function parseNavigationLabels(value: unknown): readonly NavigationLabelData[] {
  ...
  for (const expectedId of NAVIGATION_LABEL_IDS) if (!byId.has(expectedId)) throw new Error(`missing required navigation label id: ${expectedId}`);
  if (parsed.length !== NAVIGATION_LABEL_IDS.length) throw new Error(`navigation label count must remain ${NAVIGATION_LABEL_IDS.length}; got ${parsed.length}`);
  return Object.freeze(NAVIGATION_LABEL_IDS.map((id) => Object.freeze({ ...byId.get(id)! })));
}

const rawNavigationLabels: unknown = navigationJson;
export const navigationLabels = parseNavigationLabels(rawNavigationLabels);

export function getNavigationLabel(id: NavigationLabelId, source: readonly NavigationLabelData[] = navigationLabels): string {
