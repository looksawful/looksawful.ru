...
export const jesteiIntro = {
  head: {
    type: "logo",
    logoUsageId: "jestei-case-head-logo",
    wrapper: "none",
  },
...
export const jesteiBrandIntro = {
  title: "Айдентика / бренд-система",

  bodyClassName: "brand-system__intro",

  paragraphs: [
    "Мы разделили аудиторию на три группы: клубных диджеев, ивент-диджеев и саунд-продюсеров. Это позволило точнее собирать продуктовые предложения, навигацию и коммуникацию под разные сценарии работы с сервисом.",
  ],
} as const satisfies SectionIntroData;

export const jesteiInterfaceIntro = {
  title: "Интерфейс Jestei Pool",

  bodyClassName: "jestei-section-copy-list",

  paragraphs: [
    "Мы начали группировать плейлисты и сопровождать их описаниями и заголовками, наняли редактора и вместе с ним и диджеями, которые создают плейлисты, описали более 200 плейлистов на сайте, рассказали о каждом из жанров и собрали для каждой группы плейлистов свой визуальный код, вместо того чтобы использовать однообразные стоковые картинки для их обложек.",
    "Мы разработали алгоритмические плейлисты, которые автоматически собирают самые популярные треки по жанру или части мероприятия, которые пользователь ещё не слышал. Например, 10 незнакомых, но популярных треков для праймтайма или афтерпати.",
    "Мы полностью переделали сценарий покупки подписки, описали разницу между тарифами, разделили тарифы для разных сегментов при помощи цветовых профилей.",
  ],
} as const satisfies SectionIntroData;

export const jesteiEditorialIntro = {
  title: "Редполитика",

  paragraphs: [
    "Собрали единые правила для tone of voice, терминологии, UX-writing, интерфейсных текстов и редакционной работы Jestei Pool.",
  ],
} as const satisfies SectionIntroData;

export const jesteiRedpolitikaMockup = {
  entryId: "jestei-redpolitika-preview-use-01",
...
export const jesteiPromoIntro = {
  title: "Промокоммуникация",

  paragraphs: [
    "Для плейлистов, регулярных рубрик и сезонных подборок мы отказались от однообразного стокового контента и собрали собственный визуальный язык: использовали устойчивые метафоры, коллажи, иллюстрации, нейросети и отдельные стилистические решения для разных сценариев.",
  ],
} as const satisfies SectionIntroData;
