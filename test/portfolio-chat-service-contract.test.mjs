...
async function loadService() {
  assert.equal(existsSync(serviceUrl), true, "portfolio chat service must exist");
...
test("free-form input without approved evidence returns no_data without calling the provider", async () => {
  ...
  const result = await service.reply({ message: "Как ты относишься к космической архитектуре?", locale: "ru", context: { page: "home" } });
...
test("approved free-form input calls the provider once with only routed safe context", async () => {
  ...
  const result = await service.reply({
    ...
    locale: "ru",
    context: { page: "jestei", approvedSourceIds: ["attacker.injected"], email: "private@example.com", formMessage: "secret form draft" },
  });
  assert.equal(providerCalls, 1);
  assert.equal(providerInput.kind, "generate");
  assert.deepEqual(providerInput.context.sourceIds, ["project.jestei"]);
  ...
  assert.deepEqual(result, { kind: "generated", text: "Короткий ответ.", sourceIds: ["project.jestei"] });
});

test("provider failure becomes unavailable instead of rejecting the Hub request", async () => {
  const { createPortfolioChatService } = await loadService();
  const service = createPortfolioChatService({
    provider: { async generate() { throw new Error("provider offline"); } },
    router: await approvedRouter(["project.jestei"]),
  });
  const result = await service.reply({ message: "Расскажи подробнее про дизайн продукта", locale: "ru", context: { page: "jestei" } });
...
test("blank provider response becomes unavailable instead of an empty generated answer", async () => {
  const { createPortfolioChatService } = await loadService();
  const service = createPortfolioChatService({
    provider: { async generate() { return { text: "   \n  " }; } },
    router: await approvedRouter(["project.jestei"]),
  });
  const result = await service.reply({ message: "Как этот подход масштабируется на необычный сценарий?", locale: "ru", context: { page: "jestei" } });
  assert.deepEqual(result, { kind: "unavailable" });
});
...
test("transport-style answer becomes a generated service result", async () => {
  const { createPortfolioChatService } = await loadService();
  const service = createPortfolioChatService({
    provider: { async generate() { return { kind: "answer", text: "Ответ из public backend.", sources: ["project.jestei"] }; } },
    router: await approvedRouter(["project.jestei"]),
  });
  const result = await service.reply({ message: "Как устроено решение для сложного сценария?", locale: "ru", context: { page: "jestei" } });
  assert.deepEqual(result, { kind: "generated", text: "Ответ из public backend.", sourceIds: ["project.jestei"] });
});
...
test("transport-style no_data and rate_limited states survive orchestration", async () => {
  ...
  const input = { message: "Как устроено решение для сложного сценария?", locale: "ru", context: { page: "jestei" } };
  const noDataService = createPortfolioChatService({ provider: { async generate() { return { kind: "no_data", text: "", sources: [] }; } }, router });
  assert.deepEqual(await noDataService.reply(input), { kind: "no_data" });
  const limitedService = createPortfolioChatService({ provider: { async generate() { return { kind: "rate_limited" }; } }, router });
...
test("transport-style unavailable state survives orchestration", async () => {
  const { createPortfolioChatService } = await loadService();
  const service = createPortfolioChatService({
    provider: { async generate() { return { kind: "unavailable" }; } },
    router: await approvedRouter(["project.jestei"]),
  });
  const result = await service.reply({ message: "Как устроено решение для сложного сценария?", locale: "ru", context: { page: "jestei" } });
  assert.deepEqual(result, { kind: "unavailable" });
...
test("approved free-form strips PII-bearing page metadata before provider context", async () => {
  ...
  let providerInput = null;
  const service = createPortfolioChatService({
    provider: { async generate(input) { providerInput = input; return { text: "Короткий ответ." }; } },
    router: await approvedRouter(["project.jestei"]),
  });
  const result = await service.reply({
    message: "Как этот подход работает в сложном сценарии?",
    locale: "ru",
    context: { page: "https://looksawful.ru/work/jestei/?email=private@example.com#secret", email: "private@example.com" },
  });
