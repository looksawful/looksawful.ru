...
import { getEnabledSitePages, getPageByPath, sitePages } from "../src/site/pages/manifest.ts";
import { normalizePagePath, validateSitePages } from "../src/site/pages/validation.ts";
...
test("site page manifest validates without errors", () => assert.doesNotThrow(() => validateSitePages(sitePages)));
test("managed MPA routes are stable and unique", () => {
  assert.equal(sitePages.length, expectedRoutes.size);
  assert.equal(new Set(sitePages.map((page) => page.id)).size, sitePages.length);
...
test("enabled page lookup uses canonical normalized paths", () => {
  ...
  assert.equal(normalizePagePath("/blog"), "/blog/");
  assert.equal(normalizePagePath("/404.html"), "/404.html");
  assert.equal(getPageByPath("/work/jestei-pool")?.id, "case:jestei-pool");
  assert.equal(getPageByPath("/blog")?.id, "blog");
});
...
test("public case, collection and blog index pages are indexable while selected projects stay unlisted", () => {
  for (const page of sitePages) {
    if (page.type === "case" || page.type === "collection" || page.type === "blog-index") {
