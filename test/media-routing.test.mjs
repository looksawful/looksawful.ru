...
test("CMS media routes registered image replacement through affected-only generation", async () => {
  ...
  assert.match(workflow, /affected-media\.mjs/);
  ...
  const noVideoCache = step(workflow, "Require exact base cache for broad non-video media mutation");
...
test("media metadata cannot result in zero Actions while Fast CI keeps it off the engineering push path", async () => {
  ...
  for (const path of [
    ...
    assert.ok(fast.includes(path), `Fast CI push may keep ignoring ${path} only because CMS media owns it`);
  }
});
