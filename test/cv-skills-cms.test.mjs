...
const expectedSkills = {
  ...
  tech: {
    ...
    rows: [
      ...
      { id: "development", label: "Разработка:", text: "HTML • CSS • JavaScript • TypeScript • Python • VS Code • WebStorm • Zed • WebGL • GLSL • Canvas • SVG • GSAP" },
      { id: "graphics-3d", label: "3D и компьютерная графика:", text: "Blender • Maya • ZBrush • Material Designer • Three.js" },
      { id: "images", label: "Изображения:", text: "Photoshop • Lightroom • Capture One • ImageMagick" },
      ...
      { id: "motion", label: "Моушен:", text: "After Effects • Premiere Pro • Final Cut Pro • FFmpeg • Ableton Live • Adobe Audition" },
    ],
  },
  soft: {
    visible: false,
...
test("CV skill transform fails closed when source paragraph structure drifts", async () => {
  ...
  const drifted = sourceHtml.replace(
    /(<section class="block tech">[\s\S]*?)<p><b>Моушен:<\/b>[\s\S]*?<\/p>(<\/section>)/,
    "$1$2",
  );
