...
test("generic media-group base remains at the historical later cascade boundary", () => {
  assert.doesNotMatch(media, /(?:^|\n)\.media-group\s*\{/);
  assert.match(
    components,
    /\.media-group\s*\{[\s\S]*?--group-gap:\s*var\(--project-media-gap\);[\s\S]*?container:\s*media-group\s*\/\s*inline-size;/,
  );
});
