...
test("site metadata validation rejects a missing favicon asset", async () => {
  ...
  try {
    ...
    await writeFile(path.join(dir, "apple-touch-icon.png"), PNG_BYTES);
    await writeFile(path.join(dir, "favicon.svg"), '<svg xmlns="http://www.w3.org/2000/svg"></svg>', "utf8");
    await writeFile(path.join(dir, "site.webmanifest"), JSON.stringify({ theme_color: "#ffffff", icons: [{ src: "/favicon.png", sizes: "120x120", type: "image/png" }] }), "utf8");
...
test("production discovery health checks favicon and Apple icon as YandexBot", async () => {
