...
test("media-group foundation first safe slice moves only media-group substructure to the canonical media owner", () => {
  ...
  // media-group foundation still guards the substructure and specialization boundaries.
  assert.match(media, /(?:^|\n)\.media-group\s*\{/);
...
test("media-group foundation boundary still excludes neighboring authored specializations", () => {
  // Later ownership waves may move their own isolated layout families into
  // media.css. media-group foundation guards only the boundaries it actually owns.
