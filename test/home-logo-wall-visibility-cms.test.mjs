...
test("Homepage section visibility keeps stable identities and published content state", async () => {
  ...
  assert.equal(isHomeSectionVisible("client-logo-wall"), true);
...
test("enabled Homepage sections are present in generated output", async () => {
  const indexHtml = await readFile(indexUrl, "utf8");
  const rendered = renderHomepage(indexHtml);

  assert.match(rendered, /portfolio-showcase--clients/);
  assert.match(rendered, /id="portfolio-clients-title"/);
  assert.match(rendered, /portfolio-logo-wall/);
