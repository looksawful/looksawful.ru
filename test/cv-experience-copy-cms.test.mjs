...
test("CV experience transform changes editorial copy while preserving code-owned hrefs and card classes", async () => {
  ...
  const edited = clone(source);
  edited.experience[0].company = "JESTEI & POOL";
  edited.experience[0].context = "Музыкальный <сервис>";
  edited.experience[0].period = "2024–2027";
  edited.experience[0].role = "Арт-директор & дизайн-лид";
  edited.experience[0].description = "Описание <без HTML> & безопасно";
  edited.experience[0].cases[0] = "Кейс & исследование";
  edited.experience[0].links[0] = "Сайт & продукт";
  ...
  assert.match(transformed, /experience-card experience-card--jestei/);
  assert.match(transformed, /JESTEI &amp; POOL/);
  assert.match(transformed, /Музыкальный &lt;сервис&gt;/);
  assert.match(transformed, /Арт-директор &amp; дизайн-лид/);
  assert.match(transformed, /Описание &lt;без HTML&gt; &amp; безопасно/);
