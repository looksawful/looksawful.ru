...
test("Berserk Timer is a standalone final accordion scene, not an Awful Tools preview", async () => {
  ...
  const listEnd = html.indexOf(
    '<div aria-hidden="true" class="cv-accordion__progress"></div>',
    listStart,
  );
  const berserkStart = html.indexOf('<span class="cv-item__project">Berserk Timer</span>');
  assert.ok(listStart >= 0 && listEnd > listStart && berserkStart > listStart);

  const list = html.slice(listStart, listEnd);
  const projectNames = [
    ...list.matchAll(/<span class="cv-item__project">([\s\S]*?)<\/span>/g),
  ].map(([, name]) => name.trim());

  assert.equal(projectNames.at(-1), "Berserk Timer");

  const berserkArticleStart = list.lastIndexOf("<article", list.indexOf("Berserk Timer"));
  const berserkOpeningTag = list.slice(
    berserkArticleStart,
    list.indexOf(">", berserkArticleStart) + 1,
  );

  assert.equal(berserkOpeningTag.includes("data-cv-theme"), false);
