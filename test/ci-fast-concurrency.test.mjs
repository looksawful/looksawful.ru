...
test("Fast CI cancels stale revisions within the same pull request", async () => {
  const source = await fastWorkflow();
...
test("Quality does not call the retired CV postbuild hook", async () => {
  const source = await qualityWorkflow();
  assert.doesNotMatch(source, /cv:content:apply/);
});
