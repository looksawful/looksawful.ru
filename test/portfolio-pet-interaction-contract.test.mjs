...
test("PET-015..020/DR-001: click and drag are distinguishable and dragged pet stays recoverable", async () => {
  const { classifyPetGesture, clampPetPosition } = await loadRequired(
    interactionUrl,
  ...
  assert.equal(
    shiftedViewport.x,
    24 + 14 - (150 - 72),
    "visual viewport left origin and safe area must both participate in clamp",
  );
  assert.equal(
    shiftedViewport.y,
    40 + 12 - (170 - 96),
    "visual viewport top origin and safe area must both participate in clamp",
  );
});
