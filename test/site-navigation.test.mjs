...
test("blog index is marked as the current public destination", () => {
  const html = renderSiteNavigation(page("blog"));
  const blogLabel = escapeRegExp(escapeHtml(requireLabel("blog")));

  assert.match(
    html,
    new RegExp(`href="/blog/"[^>]+aria-current="page"[^>]*>${blogLabel}<`),
  );
  assert.match(
    html,
    new RegExp(`aria-current="page"[^>]*>${blogLabel}<\/span>`),
  );
});
