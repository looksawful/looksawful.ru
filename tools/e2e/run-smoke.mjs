...
async function verifyGalleryAccessibilityBaseline(page) {
  ...
  const heading = page.locator("main#main-content h1.gallery__title");
  assert.equal(await heading.count(), 1, "Gallery must expose one visible h1");
  assert.equal((await heading.innerText()).trim(), "Gallery");
