...
export function renderMediaCatalogImportIndex({
  registeredFilenames,
  uploadedFilenames,
  preserveInputOrder = false,
}) {
  const registered = preserveInputOrder
    ? [...registeredFilenames]
    : [...registeredFilenames].sort();
  const uploaded = preserveInputOrder
    ? [...uploadedFilenames]
...
function generatedImportOrder(contents, kind) {
  if (!contents) return [];
  const prefix = kind === "registered" ? "registered" : "uploaded";
  const directory = kind === "registered" ? "registered" : "uploads";
  const pattern = new RegExp(
    `import ${prefix}\\d+ from "\\.\\.\\/\\.\\.\\/content\\/media-catalog\\/${directory}\\/([^"]+)"`,
    "g",
  );
  return [...contents.matchAll(pattern)].map((match) => match[1]);
}

function preserveGeneratedOrder(currentContents, filenames, kind) {
  const expected = new Set(filenames);
  const previous = generatedImportOrder(currentContents, kind)
...
export async function syncMediaCatalog({ repoRoot = process.cwd(), check = false, checkStored = false } = {}) {
  ...
  const currentGeneratedIndex = await readFile(generatedIndexPath, "utf8").catch(() => "");
  const generated = renderMediaCatalogImportIndex({
    registeredFilenames: preserveGeneratedOrder(
      currentGeneratedIndex,
      expectedRegisteredFilenames,
      "registered",
    ),
    uploadedFilenames: preserveGeneratedOrder(
      currentGeneratedIndex,
      uploadedFilenames,
      "uploaded",
    ),
