...
Make `/cv/` the primary measurable recruiter landing while preserving privacy and the existing analytics architecture. Distinguish controlled recruiter traffic from portfolio discovery without storing recruiter names, handles, email addresses, company/person pairs or message text.
...
| HH profile | `hh` | `profile` | `job_search` or `portfolio` |
...
| LinkedIn profile | `linkedin` | `profile` | `job_search` or `portfolio` |
...
| VK DM | `vk` | `dm` | `job_search` or `portfolio` |
...
2. `cv_engaged` once per page after meaningful engagement (30 seconds of visible engagement OR meaningful scroll), for analysis rather than a primary conversion;
3. `cv_project_open` when a CV portfolio/case link is opened, with bounded target case ID/path while retaining the aggregate `project_open` event;
...
- controlled recruiter and portfolio URLs retain UTM parameters on landing;
- CV semantic events fire at most once per intended action;
- a CV project click contributes to both aggregate `project_open` and CV-specific `cv_project_open`;
- the 30-second engagement trigger counts visible engagement time rather than a hidden background tab;
...
- private reporting groups `job_search` and `portfolio` by source, medium, content, opaque batch and landing page.
