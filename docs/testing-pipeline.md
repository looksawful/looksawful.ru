...
Текущий Fast содержит 89 дешёвых долгоживущих contract:

The canonical Fast allowlist is `fastTests` in `tools/ci/run-tests.mjs`; this document intentionally does not duplicate the full filename list, because that would become a stale second registry. The verified 2026-09-15 `dev` baseline contains 89 Fast contracts.
...
### Affected image media path

Для PR, содержащего только зарегистрированные raster source images, Fast CI остаётся обязательным gate, но после дешёвой классификации пропускает dependency install, global media fingerprint/cache recovery, ffmpeg, `media:sync`, typecheck, Fast tests и production build. Физическую media-работу выполняет `.github/workflows/media-affected.yml`: он резолвит точные MediaAsset IDs и генерирует responsive variants только для затронутых изображений. Push в `dev` использует тот же affected-only принцип в `CMS media`; exact previous whole-library cache для такого image-only изменения не требуется.

Full media path остаётся для mixed code/media diffs, media tooling/config/registry changes, video work и explicit manual/scheduled rebuilds.

Контрольный прогон 2026-09-06 для одного `public/media/projects/index/shootings-cover.webp` подтвердил маршрутизацию: `1` source, `3` responsive variants, `93,012` generated bytes, responsive build `1s`; ffmpeg, video tooling и whole-library `media:sync` не запускались. Fast CI на том же PR выполнил классификацию и пропустил все тяжёлые media/cache/build steps. Реализация: #394 / PR #436; validation run: PR #437.
