...
На момент этой инструкции GitHub default branch репозитория всё ещё `prod`, поэтому перед редактированием проверь выбранную в Pages CMS ветвь и при необходимости переключись на `dev`.
...
`Проверить сайт` запускает существующий fast verification flow для текущей выбранной ветви. Проверка ничего не публикует.
