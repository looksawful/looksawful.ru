...
const resolveSceneId = (sceneId) =>
	SHOWCASE_ANIMATION_SCENES[sceneId] ? sceneId : DEFAULT_MASONRY_SCENE;
...
const getSceneItems = (sceneId) => {
  ...
  const scene = SHOWCASE_ANIMATION_SCENES[resolvedSceneId];
...
export const mountMasonry = async (canvasId = "masonry-container", options = {}) => {
	...
	const sceneId = resolveSceneId(options.scene || canvas.dataset.showcaseAnimationScene);
