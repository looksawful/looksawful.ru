...
const createLightbox = () => {
  ...
  lightbox.className = "showcase-media-lightbox";
  lightbox.hidden = true;
  lightbox.innerHTML = `
    <button class="showcase-media-lightbox__close" type="button" aria-label="закрыть">×</button>
    <div class="showcase-media-lightbox__stage" role="dialog" aria-modal="true"></div>
  `;

  lightbox.addEventListener("click", (event) => {
    if (event.target === lightbox || event.target.closest(".showcase-media-lightbox__close")) {
      closeLightbox();
    }
  });
...
const openLightbox = (item, extension) => {
  ...
  const stage = lightbox.querySelector(".showcase-media-lightbox__stage");
  ...
  document.documentElement.classList.add("has-showcase-media-lightbox");
};
...
function closeLightbox() {
  ...
  activeLightbox.querySelector(".showcase-media-lightbox__stage").textContent = "";
  activeLightbox.hidden = true;
  document.documentElement.classList.remove("has-showcase-media-lightbox");
}
...
function initStaticMediaLightboxLinks(root = document) {
  ...
  scope.addEventListener("click", (event) => {
    const link = event.target?.closest?.('a[data-showcase-lightbox="media"]');
