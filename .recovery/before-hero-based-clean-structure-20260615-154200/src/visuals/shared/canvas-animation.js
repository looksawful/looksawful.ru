...
export const getCanvasMountOptions = (canvas, options = {}, defaults = {}) => {
  ...
  const quality = options.quality || dataset.showcaseAnimationQuality || getCanvasQualityProfile();
  ...
  return {
    quality,
    maxDpr: Number(options.maxDpr ?? dataset.showcaseAnimationDpr ?? getProfileDefault("maxDpr")),
    maxItems: numberFrom(options.maxItems ?? dataset.showcaseAnimationMaxItems, getProfileDefault("maxItems"), 1, 160),
    initialCount: numberFrom(
      options.initialCount ?? dataset.showcaseAnimationInitialCount,
      getProfileDefault("initialCount"),
      1,
      80,
    ),
    batchSize: numberFrom(options.batchSize ?? dataset.showcaseAnimationBatchSize, getProfileDefault("batchSize"), 1, 32),
    fps: numberFrom(options.fps ?? dataset.showcaseAnimationFps, getProfileDefault("fps"), 1, 60),
    speedScale: Number(options.speedScale ?? dataset.showcaseAnimationSpeed ?? defaults.speedScale ?? 1) || 1,
  };
};
...
export const markCanvasState = (canvas, state) => {
  const host = canvas?.closest?.("[data-showcase-animation], .media-preview, .embedded-demo") || canvas?.parentElement;
