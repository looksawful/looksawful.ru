...
function drawFace(context, state, metrics) {
    ...
    context.save();
    context.translate(state.pointerX * 2.5 + idleX, state.pointerY * 1.7 + state.scroll * 3.2 + idleY);
    ...
    const leftEyeX = ((pointerFaceX - EYES.left.x) / FACE_HALF) *
        EYE_TRACK_STRENGTH_X *
        pointerInfluence;
    const leftEyeY = ((pointerFaceY - EYES.left.y) / FACE_HALF) *
        EYE_TRACK_STRENGTH_Y *
        pointerInfluence;
    const rightEyeX = ((pointerFaceX - EYES.right.x) / FACE_HALF) *
        EYE_TRACK_STRENGTH_X *
        pointerInfluence;
    const rightEyeY = ((pointerFaceY - EYES.right.y) / FACE_HALF) *
        0.5 *
        EYE_TRACK_STRENGTH_Y *
        pointerInfluence;
...
export function createAwfulHeadMotion({ root, canvas, }) {
    ...
    const syncTicker = () => {
        const shouldRenderContinuously = visible && !reducedMotion.matches;
        if (shouldRenderContinuously && !tickerActive) {
            gsap.ticker.add(render);
            tickerActive = true;
        }
        else if (!shouldRenderContinuously && tickerActive) {
